        ...
        <div className="mb-10 grid grid-cols-1 md:grid-cols-3 gap-6 items-end mt-12">
            ...
            >
                <Link href={`/tool/${second.slug}`} className="block">
                    <div className="bg-card rounded-2xl p-6 relative border border-border hover:border-purple-500/50 transition-all duration-300 flex flex-col items-center group cursor-pointer hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(139,92,246,0.15)]">
                        {/* Rank Badge */}
                        ...
                        {/* Name */}
                        <h3 className="text-xl font-bold mb-2 text-foreground text-center group-hover:text-purple-300 transition-colors line-clamp-1">
                            {second.name}
                        </h3>
            ...
            >
                <Link href={`/tool/${first.slug}`} className="block">
                    <div className="bg-card rounded-2xl p-8 relative border-2 border-primary/40 shadow-[0_0_40px_rgba(6,249,188,0.2)] transform md:-translate-y-6 flex flex-col items-center group cursor-pointer hover:scale-[1.02] hover:shadow-[0_0_60px_rgba(6,249,188,0.3)] transition-all duration-300">
                        {/* Crown Animation */}
                        <motion.div
                            animate={{ y: [0, -5, 0] }}
                        ...
                        >
                            <Crown className="w-10 h-10 text-primary fill-primary/20 drop-shadow-[0_0_10px_rgba(6,249,188,0.5)]" />
                        </motion.div>

                        {/* Rank Badge */}
                        <div className="absolute -top-5 mt-3 bg-primary text-primary-foreground font-black px-5 py-2 rounded-full text-base shadow-[0_0_20px_rgba(6,249,188,0.4)] flex items-center gap-1.5">
                        ...
                        {/* Name */}
                        <h3 className="text-2xl font-bold mb-2 text-foreground text-center group-hover:text-primary transition-colors line-clamp-1">
                            {first.name}
                        </h3>

                        {/* Score */}
                        <div className="flex items-baseline gap-1.5">
                            <span className="text-primary font-black text-5xl drop-shadow-[0_0_10px_rgba(6,249,188,0.3)]">{first.frugal_score_total}</span>
                            <span className="text-sm text-muted-foreground font-medium">pts</span>
                        </div>

                        {/* Champion Badge */}
                        <div className="mt-4 text-sm text-primary font-bold bg-primary/10 px-4 py-1.5 rounded-full border border-primary/30 flex items-center gap-2">
                            <Sparkles className="w-4 h-4" />
                            Most Efficient
                        </div>

                        {/* Podium bar */}
                        <div className="absolute bottom-0 left-0 right-0 h-3 bg-gradient-to-r from-primary/60 via-emerald-500/60 to-primary/60 rounded-b-2xl" />
                    </div>
                </Link>
            </motion.div>
            ...
            >
                <Link href={`/tool/${third.slug}`} className="block">
                    <div className="bg-card rounded-2xl p-6 relative border border-border hover:border-orange-500/50 transition-all duration-300 flex flex-col items-center group cursor-pointer hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                        {/* Rank Badge */}
                        ...
                        {/* Name */}
                        <h3 className="text-xl font-bold mb-2 text-foreground text-center group-hover:text-orange-300 transition-colors line-clamp-1">
                            {third.name}
                        </h3>
